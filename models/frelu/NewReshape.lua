local NewReshape, parent = torch.class('nn.NewReshape', 'nn.Module')

function NewReshape:__init(...)
	parent.__init(self)
	local arg = {...}
...
function NewReshape:updateOutput(input)
	...
	if (self.batchMode == false) or (
		...
		for i=1,#self.size do
			if self.size[i] == 0 then
				self.size[i] = input:size(i)
			end
		end
		self.output:view(input, self.size)
	else
		self.batchsize[1] = input:size(1)
		for i=2,#self.batchsize do
			if self.batchsize[i] == 0 then
				self.batchsize[i] = input:size(i)
			end
		end
...
function NewReshape:updateGradInput(input, gradOutput)
...
function NewReshape:__tostring__()
	return torch.type(self) .. '(' ..
	  table.concat(self.size:totable(), 'x') .. ')'
end

function NewReshape:clearState()
